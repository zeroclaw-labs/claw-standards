...
## §3 Peter: your plugin path is the route, bundled by default

I am not asking you to reverse #6842, and I am not asking you to staff anything. Your close was a plugin-scope decision ("better suited for ClawHub/community plugin work"), grounded in VISION.md's lean-core lines, including the two that matter most: "If you build a plugin, host and maintain it in your own repository" (openclaw@203a896:VISION.md#L78) and plugin promotion "belongs in ClawHub, preferably under vetted org publishers for official plugins" (#L82). The spec's component provision (§3.4) stays inside that: a unit is satisfied by a first-party component or a designated external one, so an official A2A plugin (the #6842 thread already links @a2anet/openclaw-a2a-plugin and openclaw-a2a-gateway) with a passing TCK run in `claw-conformance.json` is the route. Package name, hosting, and maintenance stay your calls, not spec text.

One honest addition, and it is the only real ask in this section: the blessed plugin has to ship in the default OpenClaw distribution, enabled by one setting, rather than sit in ClawHub as an opt-in. The reason is the whole point of the exercise. If A2A is something a user has to go find and install, most deployed OpenClaw agents will not speak it, and "OpenClaw supports A2A" stops meaning "an OpenClaw agent you meet can talk to mine." Bundled and off by default keeps core lean and keeps you in control of exposure; it only guarantees the capability is present to switch on. That is a bigger commitment than "a plugin exists somewhere," and I think it is the one that actually buys interop.
...
The honest catch, and it is the same one §3.4(a) puts to everyone: a sidecar only counts if it ships in NanoClaw's default or recommended setup (an official `setup/add-acp` step on `main`), not as an optional add a user has to hunt for. That is the requirement most in tension with keeping the core small enough to read, and whether it is worth it, or whether you take U-SKILLS alone and stop there, is entirely your call. U-SKILLS by itself is a complete, legitimate claim.
