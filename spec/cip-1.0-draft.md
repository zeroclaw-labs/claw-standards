...
### 3.4 Component provision (core, plugin, or sidecar)

A unit MAY be satisfied by a plugin, adapter, or sidecar rather than by core code. CIP does not care where the implementation lives. It cares that a normal deployment actually exposes the capability, so that "harness X is conformant for unit U" means "an agent someone actually runs on harness X can interoperate over U," not merely "an implementation of U exists somewhere." A component-based implementation satisfies a unit IF all of the following hold:

- (a) **Shipped in the default distribution.** The component is part of what a user gets from the harness's standard install or documented default setup, not a separate item they must discover and acquire from a marketplace or third-party repository. A network-facing capability MAY be left off until the operator sets one documented configuration flag (a harness MAY keep such a capability disabled by default for security), but enabling it MUST NOT require installing additional software beyond the standard distribution. "An official plugin exists in the marketplace" does NOT satisfy this. "The official plugin ships with the standard install and is enabled by one documented setting" does. The distinction is deliberate: a capability a running agent might not have installed is not interop.
- (b) **First-party or designated.** The component is maintained by the harness project (the same org or owner account as the harness repo; a personal-account repo qualifies), OR it is an external component that the harness's own docs or manifest name as the blessed default for that capability (for example an official ClawHub-published plugin under a vetted publisher, per OpenClaw's VISION.md: "plugin promotion belongs in ClawHub, preferably under vetted org publishers for official plugins", openclaw@203a896:VISION.md#L82).
- (c) **CI-verified against a pinned harness release**, either in the harness's own CI or in the component's own CI pinned to the harness release tag, with the passing run URL feeding the manifest (§3.5). Trunk-CI integration in the harness repo is NOT required. The verified instance MUST be brought up from the harness's documented default or recommended setup (with the capability's config flag enabled where applicable), never a special build that exists only for the test.
- (d) **Owns its own state exclusively:** it MUST NOT create tables in, or otherwise change the schema of, the harness's own data stores.

This provision preserves modularity without weakening interop. A lean core (OpenClaw), a core small enough to read (NanoClaw), and sidecar isolation are all fully compatible with it: bundle the component in the standard distribution and let one config flag turn it on. Where the code lives stays the maintainer's call; that a standard install carries it is the requirement. This is the load-bearing change from earlier drafts of this document, which accepted any documented optional component and so let "an A2A plugin exists in ClawHub" count as conformance even if almost no running agent had installed it.

For OpenClaw specifically: "Core stays lean; optional capabilities should usually ship as plugins" (openclaw@203a896:VISION.md#L63-L66) and the ClawHub promotion path (#L82) stay the intended route. What (a) adds is that the blessed A2A plugin ships in the default OpenClaw distribution rather than sitting in the marketplace as an opt-in. When Peter closed openclaw#6842 as "better suited for ClawHub/community plugin work" (steipete, 2026-04-26, evidence/openclaw-issue-6842-comments.json), the thread already linked working external implementations (openclaw-a2a-gateway and @a2anet/openclaw-a2a-plugin). Designating one of those, or building first-party, bundling it by default, and pointing `claw-conformance.json` at its passing TCK run is a fully conformant route to U-A2A-SERVER. Package, hosting, and maintenance stay the maintainer's calls, not spec text.

### 3.5 The manifest: `claw-conformance.json`

A harness claiming any unit MUST publish a `claw-conformance.json` at a documented, discoverable path of its choosing — repo root, `.github/`, `docs/`, or a per-harness page in claw-standards — regenerated per release, recording per-unit status, how the unit is satisfied (`core`, `plugin`, or `sidecar`), for a non-`core` component a `ships_in_default_distribution` confirmation per §3.4(a), the suite version, and a **public CI run URL** as evidence. The badge action (W-N2) takes the path as configuration. Schema sketch in Appendix A.
...
| U-A2A-SERVER | None native — no agent card, no A2A JSON-RPC anywhere in src/packages/extensions; all "a2a" hits are the internal same-gateway `sessions_send` flow. #6842 closed 2026-04-26 as ClawHub/community plugin scope (quoted §3.4), and the thread links working external implementations (openclaw-a2a-gateway; @a2anet/openclaw-a2a-plugin, tested end-to-end per its author). The plugin API demonstrably covers the needed surface: `registerHttpRoute` + `registerTool`. | OC-A2A-1 (grep results); evidence/openclaw-issue-6842-comments.json; OC-PLUGIN-1 | A first-party **or designated external** plugin (§3.4), **bundled in the default OpenClaw distribution** and enabled by one documented setting (§3.4(a)) — not a marketplace opt-in. Designating an existing community plugin (or building first-party), shipping it by default, and pointing `claw-conformance.json` at its passing TCK run is the conformant route. Package, hosting, and whether at all are Peter's calls; that a default install can speak A2A is the bar |
| U-A2A-CLIENT | None (same absence evidence). | OC-A2A-1 | Client side of the same default-shipped (first-party or designated) component |

### 6.3 NanoClaw (outside reading — please correct)

NanoClaw's product plane passes U-SKILLS validation today — its Phase 1 is a CI job, nothing more. Its protocol route is sidecars at the channel-ingress plane. Honest flag on §3.4(a): "shipped in the default distribution" is the requirement most in tension with NanoClaw's fork-and-add minimalism, because for NanoClaw "default distribution" means the documented default/recommended setup path (an official `setup/add-acp` / `add-a2a` step that ships on `main`), not a marketplace. A protocol unit counts for NanoClaw only if that sidecar is part of the recommended setup, not an optional extra a user has to find. Whether NanoClaw wants any protocol unit at all, given that bar, is entirely Gavriel's call — U-SKILLS alone is a complete, legitimate claim (§3.2).
...
| OC-3 | A2A via §3.4: either designate an external plugin (the #6842 thread already links openclaw-a2a-gateway and @a2anet/openclaw-a2a-plugin) or build first-party (`registerHttpRoute` card + JSON-RPC endpoint, `registerTool` for outbound); **bundle it in the default distribution** (§3.4(a)) so a standard install speaks A2A, and point the manifest at its passing TCK run. Route, naming, hosting: Peter's calls | U-A2A-* | 2–3 |
  ...
  "$defs": {
    "unit": {
      ...
      "properties": {
        ...
        "ships_in_default_distribution": { "type": "boolean", "description": "REQUIRED when mode != core: the component is part of the standard install / documented default setup and is enabled by at most one documented config flag, not a marketplace opt-in (§3.4(a)). MUST be true for a pass claim on a non-core unit." },
